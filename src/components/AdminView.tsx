...
export function AdminView() {
  ...
  const passwordError = touched && password.trim() === '' ? 'Password is required.' : ''
    ...
    <section className="av-section">
      <div className="pf-container">
        <div className="av-card">
          ...
          <form onSubmit={handleSubmit} noValidate className="av-form">
            <div className="av-input-row">
              <div className="av-field">
                ...
                {passwordError && (
                  <span id="av-pw-err" className="av-field-error" role="alert">{passwordError}</span>
                )}
                {status === 'error' && errorMsg && (
                  <span className="av-field-error" role="alert">{errorMsg}</span>
                )}
              </div>
              ...
              >
                {status === 'loading' ? (
                  <><span className="av-spinner" aria-hidden="true" /> Verifying…</>
                ) : (
                  <>
                    <svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" className="av-submit-icon">
          ...
          {/* — Data table — */}
          {status === 'success' && (
            <div className="av-table-wrap" role="region" aria-label="Submitted form entries">
              <div className="av-table-meta">
                <span className="av-table-count">{entries.length} submission{entries.length !== 1 ? 's' : ''}</span>
                <button
                  type="button"
                  className="av-clear-btn"
                  onClick={() => { setEntries([]); setStatus('idle') }}
                >
                  Hide data
                </button>
              </div>
                    ...
                    <tbody>
                      {entries.map((entry, i) => (
                        <tr key={entry._id}>
                          ...
                          <td>
                            <a
                              href={entry.contact.includes('@') ? `mailto:${entry.contact}` : `tel:${entry.contact}`}
                          ...
                          <td className="av-td-suggestions">
                            {entry.suggestions || <span className="av-none">—</span>}
                          </td>
                          <td className="av-td-date">{formatDate(entry.createdAt)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
